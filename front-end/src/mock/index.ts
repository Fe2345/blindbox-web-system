...
export function setupMock() {
  ...
  mock.onGet('/user/api/blindboxes').reply(() => {
    return [200, { code: 0, data: mockBlindBoxes, message: 'ok' }]
  })

  mock.onGet(/\/user\/api\/blindboxes\/.*/).reply((config) => {
    const id = config.url?.split('/').pop()
    const box = mockBlindBoxes.find((b) => b.id === id)
    if (box) return [200, { code: 0, data: box, message: 'ok' }]
  ...
  mock.onGet('/user/api/assets').reply(() => {
    return [200, { code: 0, data: mockAssets, message: 'ok' }]
  })

  mock.onGet(/\/user\/api\/assets\/.*/).reply((config) => {
    const id = config.url?.split('/').pop()
    const asset = mockAssets.find((a) => a.id === id)
    if (asset) return [200, { code: 0, data: asset, message: 'ok' }]
  ...
  mock.onPost(/\/user\/api\/assets\/.*\/recycle/).reply((config) => {
    ...
    return [200, { code: 0, data: { recycledPoints: asset.recyclablePoints }, message: '回收成功' }]
  })
  ...
  mock.onPost(/\/user\/api\/assets\/.*\/ship/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '发货申请已提交' }]
  })
  ...
  mock.onPost(/\/user\/api\/assets\/.*\/publish-exchange/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '发布成功' }]
  })

  // Orders
  mock.onGet('/user/api/orders').reply(() => {
    return [200, { code: 0, data: mockOrders, message: 'ok' }]
  })
  ...
  mock.onPost(/\/user\/api\/orders\/.*\/confirm/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '确认收货成功' }]
  })

  // Exchange
  mock.onGet('/user/api/exchange/posts').reply(() => {
    return [200, { code: 0, data: mockExchangePosts, message: 'ok' }]
  })

  mock.onGet('/user/api/exchange/applications').reply(() => {
    return [200, { code: 0, data: mockExchangeApplications, message: 'ok' }]
  })
  ...
  mock.onPost(/\/user\/api\/exchange\/applications\/.*\/accept/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '已接受' }]
  })
  ...
  mock.onPost(/\/user\/api\/exchange\/applications\/.*\/reject/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '已拒绝' }]
  })

  // Points
  mock.onGet('/user/api/points/records').reply(() => {
    return [200, { code: 0, data: mockPointsRecords, message: 'ok' }]
  })

  mock.onGet('/user/api/points/balance').reply(() => {
    return [200, { code: 0, data: { balance: 930 }, message: 'ok' }]
  })

  // Transactions
  mock.onGet('/user/api/transactions').reply(() => {
    return [200, { code: 0, data: mockTransactionRecords, message: 'ok' }]
  })
  ...
  mock.onPost('/admin/api/login').reply((config) => {
    ...
    if (username === 'admin' && password === 'admin123') {
      return [200, { code: 0, data: { token: 'admin-mock-token', user: { id: 'a001', username: '超级管理员', role: 'admin' } }, message: 'ok' }]
    }
    return [200, { code: -1, data: null, message: '用户名或密码错误' }]
  })

  mock.onGet('/admin/api/info').reply(() => {
    return [200, { code: 0, data: { id: 'a001', username: '超级管理员', role: 'admin' }, message: 'ok' }]
  })
  ...
  // Merchants
  mock.onGet('/admin/api/merchants').reply(() => [200, { code: 0, data: mockMerchants, message: 'ok' }])
  mock.onGet(/\/admin\/api\/merchants\/.*/).reply((config) => {
    const id = config.url?.split('/').pop()
    const m = mockMerchants.find((x) => x.id === id)
    return m ? [200, { code: 0, data: m, message: 'ok' }] : [404, { code: -1, data: null, message: 'not found' }]
  })
  mock.onPost(/\/admin\/api\/merchants\/.*\/review/).reply((config) => {
    ...
    return [200, { code: 0, data: null, message: '审核完成' }]
  })
  ...
  // Users
  mock.onGet('/admin/api/users').reply(() => [200, { code: 0, data: mockAdminUsers, message: 'ok' }])
  ...
  // Products
  mock.onGet('/admin/api/products').reply(() => [200, { code: 0, data: mockProducts, message: 'ok' }])
  ...
  mock.onPost('/admin/api/products').reply(() => [200, { code: 0, data: null, message: '添加成功' }])
  mock.onPut(/\/admin\/api\/products\/.*/).reply(() => [200, { code: 0, data: null, message: '修改成功' }])
  mock.onPost(/\/admin\/api\/products\/.*\/offline/).reply((config) => {
    const id = config.url?.split('/')[4]
    const p = mockProducts.find((x) => x.id === id)
    if (p) p.status = 'offline'
    return [200, { code: 0, data: null, message: '已下架' }]
  })

  // BlindBoxes
  mock.onGet('/admin/api/blindboxes').reply(() => [200, { code: 0, data: mockAdminBlindBoxes, message: 'ok' }])
  mock.onPost('/admin/api/blindboxes').reply(() => [200, { code: 0, data: null, message: '添加成功' }])
  mock.onPut(/\/admin\/api\/blindboxes\/.*/).reply(() => [200, { code: 0, data: null, message: '修改成功' }])
  ...
  mock.onPost(/\/admin\/api\/blindboxes\/.*\/prizes/).reply(() => [200, { code: 0, data: null, message: '奖池已更新' }])

  // Orders
  mock.onGet('/admin/api/orders').reply(() => [200, { code: 0, data: mockAdminOrders, message: 'ok' }])
  ...
  // Exchanges
  mock.onGet('/admin/api/exchanges').reply(() => [200, { code: 0, data: mockAdminExchanges, message: 'ok' }])
  mock.onPost(/\/admin\/api\/exchanges\/.*\/resolve/).reply((config) => {
    const id = config.url?.split('/')[4]
    const ex = mockAdminExchanges.find((x) => x.id === id)
    if (ex) ex.status = 'completed'
    return [200, { code: 0, data: null, message: '已处理' }]
  })

  // Rules
  mock.onGet('/admin/api/rules').reply(() => [200, { code: 0, data: mockRules, message: 'ok' }])
  mock.onPost('/admin/api/rules').reply((config) => {
    const data = JSON.parse(config.data)
    Object.assign(mockRules, data, { updatedAt: new Date().toISOString() })
    return [200, { code: 0, data: null, message: '保存成功' }]
  })

  // Ledger
  mock.onGet('/admin/api/ledger').reply(() => [200, { code: 0, data: mockLedger, message: 'ok' }])

  // Logs
  mock.onGet('/admin/api/logs').reply(() => [200, { code: 0, data: mockOpLogs, message: 'ok' }])

  // Exceptions
  mock.onGet('/admin/api/exceptions').reply(() => [200, { code: 0, data: mockExceptions, message: 'ok' }])
